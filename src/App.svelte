...
<main class="relative min-h-screen bg-slate-950 text-white overflow-x-hidden">
  ...
  <header class="sticky top-0 z-30 border-b border-white/5 bg-slate-950/80 backdrop-blur-xl">
    <div class="mx-auto max-w-7xl px-4 py-4 sm:flex sm:items-center sm:justify-between">
      <div class="mb-4 sm:mb-0 flex items-center justify-between">
        <div>
          ...
          <p class="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em]">{$t('header.sync')}</p>
        </div>

        <!-- Language Switcher -->
        <div class="sm:hidden flex items-center bg-white/5 rounded-lg border border-white/5 p-1 gap-1">
          {#each ['es', 'en'] as l}
            <button
              on:click={() => ($locale = l as Locale)}
              class={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md transition-all duration-300 ${
                $locale === l ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              {l}
            </button>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="hidden sm:flex items-center bg-white/5 rounded-lg border border-white/5 p-1 gap-1">
          {#each ['es', 'en'] as l}
            <button
              on:click={() => ($locale = l as Locale)}
              class={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-md transition-all duration-300 ${
                $locale === l ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'
              }`}
            >
              {l}
            </button>
  ...
  <section class="mx-auto max-w-7xl px-4 py-8 md:py-12">
    {#if isLoading}
      <div class="flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <p class="text-slate-400 font-medium animate-pulse">{$t('loading.message')}</p>
      </div>
